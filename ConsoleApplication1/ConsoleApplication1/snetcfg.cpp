...
#define _WIN32_WINNT 0x0501		// We need some XP definitions.
...
//
HRESULT HrInstallNetComponent(IN INetCfg* pnc, IN PCWSTR szComponentId, IN const GUID* pguidClass);
	...
	IN PCWSTR szInfFullPath)
{
	...
	if ((nc == NC_NetProtocol) || (nc == NC_NetService) || (nc == NC_NetClient))
	{
		...
		if (szInfFullPath && wcslen(szInfFullPath))
		{
			...
			{
				//ShowMessage(L"...%s was copied to %s", szInfFullPath, szInfNameAfterCopy);
			}
			else
			{
		...
		if (S_OK == hr)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				hr = HrInstallNetComponent(pnc, szComponentId, c_aguidClass[nc]);
				if (SUCCEEDED(hr))
				{
					// Apply the changes
					hr = pnc->Apply();
...
HRESULT HrUninstallNetComponent(IN PCWSTR szComponentId)
{
	...
	//::MessageBox(NULL, szComponentId, (LPCWSTR)"", MB_OK);
	...
	if (SUCCEEDED(hr))
	{
		...
		else if (S_FALSE == hr)
		{
			...
			//::MessageBox(NULL, (LPCWSTR)"is not installed", (LPCWSTR)"", MB_OK);
		}
...
#define MAX_COMP_DESC   4096
	...
	if (INVALID_HANDLE_VALUE != hdi)
	{
		...
		while (deid.cbSize = sizeof(SP_DEVINFO_DATA), SetupDiEnumDeviceInfo(hdi, dwIndex, &deid))
		{
			dwIndex++;
			...
			fSuccess = SetupDiGetDeviceInstanceId(hdi, &deid, szCompInstanceId, MAX_COMP_INSTID, NULL);
			if (fSuccess)
			{
...
// Notes:
//
HRESULT HrShowNetComponents(IN INetCfg* pnc, IN const GUID* pguidClass)
{
	HRESULT hr = S_OK;
	PWSTR szInfId;
	PWSTR szDisplayName;
	...
	hr = pnc->QueryNetCfgClass(pguidClass, IID_INetCfgClass, (void**)&pncclass);
	if (SUCCEEDED(hr))
	{
		...
		while (SUCCEEDED(hr) && (S_OK == (hr = pencc->Next(1, &pncc, &celtFetched))))
		{
...
// Notes:
//
HRESULT HrGetNextBindingInterface(IN  INetCfgBindingPath* pncbp, OUT INetCfgBindingInterface** ppncbi)
{
	HRESULT hr = S_OK;
...
// Notes:
//
HRESULT HrGetNextBindingPath(IN  INetCfgComponent* pncc, IN  DWORD  dwBindingPathType, OUT INetCfgBindingPath** ppncbp)
{
	HRESULT hr = S_OK;
...
HRESULT HrShowBindingPath(IN INetCfgBindingPath* pncbp)
{
	...
	while (SUCCEEDED(hr) && (S_OK == (hr = HrGetNextBindingInterface(pncbp, &pncbi))))
	{
...
HRESULT HrShowBindingPathsOfComponent(IN PCWSTR szComponentId)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (S_OK == hr)
		{
			...
			while (S_OK == (hr = HrGetNextBindingPath(pncc, EBP_BELOW, &pncbp)))
			{
			...
			while (S_OK == (hr = HrGetNextBindingPath(pncc, EBP_ABOVE, &pncbp)))
			{
				// display the binding path
				hr = HrShowBindingPath(pncbp);
				ReleaseObj(pncbp);
...
void ShowHrMessage(IN HRESULT hr)
{
	...
	{
		//ShowMessage(L"...done");
		//::MessageBox(NULL, (LPCWSTR)"done", (LPCWSTR)"", MB_OK);
		if (NETCFG_S_REBOOT == hr)
		{
			//ShowMessage(L"*** You need to reboot your computer for this change to take effect ***");
			//::MessageBox(NULL, (LPCWSTR)"*** You need to reboot your computer for this change to take effect ***", (LPCWSTR)"", MB_OK);
		}
	}
	else
	{
		//ShowMessage(L"..failed. Error code: 0x%lx", hr);
		//::MessageBox(NULL, (LPCWSTR)"..failed. Error code: 0x%lx", (LPCWSTR)"", MB_OK);
	}
}
